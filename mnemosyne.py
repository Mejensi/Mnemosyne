...
class C:
    ...
    BOLD = '\033[1m'
    DIM = '\033[2m'
    PRIMARY = '\033[38;2;91;155;213m'
    SUCCESS = '\033[38;2;16;185;129m'
    WARNING = '\033[38;2;245;158;11m'
    ERROR = '\033[38;2;239;68;68m'
    INFO = '\033[38;2;14;165;233m'
...
INFO_TICKER = [
    ...
    "◈ Diagnostic Health Check: Engine state [OPTIMAL].",
...
def ensure_ffmpeg(auto_download=True):
    ...
    ffprobe_bin = BIN_DIR / ("ffprobe.exe" if IS_WINDOWS else "ffprobe")
...
def verify_output(inp, outp):
    ...
    try:
        ...
        in_d, in_f = get_meta(inp); out_d, out_f = get_meta(outp)
...
def process_video(wid, vpath, codec, config):
    ...
    try:
        meta = get_file_metadata(vpath); start_size = vpath.stat().st_size
        ...
        while True:
            ...
            if "out_time=" in line:
                ...
                try:
                    s_fps = line.split("fps=")[1].split()[0]; s_speed = line.split("speed=")[1].split()[0]
                    worker_stats.update(wid, fn, pct, s_fps, s_speed)
                except: pass
        ...
        end_size = tmp.stat().st_size; size_diff = (1 - (end_size / start_size)) * 100
        ...
        try:
            vpath.rename(bak); tmp.rename(vpath); restore_file_metadata(vpath, meta)
...
def update_display(total, codec_name, config):
    ...
    buffer.append("\033[J") # Clear bottom
...
def show_security_notice(log_msg, drive_type=None):
    clear_screen(); w = 70
    draw_separator(w, 'top'); draw_box_line("SECURITY & TRANSPARENCY", w, C.BOLD + C.PRIMARY); draw_separator(w, 'mid')
...
def main():
    ...
    if not args.desktop_log:
        ...
        draw_box_line("[1] Desktop - Easy to find", w, C.INFO)
        draw_box_line("[2] AppData - Clean desktop", w, C.INFO)
        draw_separator(w, 'bot')
    ...
    while True:
        ...
        strat = input(f" {C.PRIMARY}>> Choice [Default=1]: {C.RESET}").strip()
        config['recursive'] = (strat == '2')
        
        print(f"\n {C.PRIMARY}[BOOT] Priority:{C.RESET}")
        print(f"    [1] Name A-Z  [2] Name Z-A  [3] Largest  [4] Smallest")
        sort_opt = input(f" {C.PRIMARY}>> Choice [Default=1]: {C.RESET}").strip()
        ...
        mode_opt = input(f" {C.PRIMARY}>> Choice [Default=2]: {C.RESET}").strip()
        if mode_opt == '1': config['max_workers'] = 1
        elif mode_opt == '3':
            try: config['max_workers'] = int(input(f" {C.PRIMARY}>> Workers (1-{os.cpu_count()}): {C.RESET}"))
            except: config['max_workers'] = DEFAULT_CONFIG['max_workers']
        else: config['max_workers'] = DEFAULT_CONFIG['max_workers']
